...
const DATA = {
  ...
  models: {
    ...
    recall:    [80.97, 91.80, 89.17],
    f1:        [77.66, 87.92, 88.10],
  },
  ...
  missing: {
    ...
    pct:    [5.21,5.21,5.21,5.21,5.51,5.21,10.17,10.17,10.17,1.19,2.90],
  },
  ...
  sampleData: [
    { crop:3, temp:0.794, hum:0.500, co2:0.668, lux:0.227, irr:0.704, N:0.512, P:0.484, K:0.435, ph:0.367, pest:0.217, y:0 },
    { crop:0, temp:0.645, hum:0.636, co2:0.509, lux:0.432, irr:0.656, N:0.553, P:0.571, K:0.271, ph:0.433, pest:0.493, y:0 },
    { crop:3, temp:0.621, hum:0.557, co2:0.555, lux:0.490, irr:0.480, N:0.906, P:0.374, K:0.959, ph:0.333, pest:0.203, y:1 },
    { crop:3, temp:0.752, hum:0.547, co2:0.425, lux:0.741, irr:0.352, N:0.512, P:0.363, K:0.906, ph:0.333, pest:0.145, y:1 },
    { crop:0, temp:0.631, hum:0.428, co2:0.673, lux:0.332, irr:0.592, N:0.129, P:0.747, K:0.547, ph:0.800, pest:0.188, y:0 },
  ],
  ...
  errors: [
    { id:'FP-1 · Baris 3905', type:'fp', actual:'Rendah', pred:'Tinggi',
    ...
    { id:'FP-2 · Baris 1849', type:'fp', actual:'Rendah', pred:'Tinggi',
    ...
    { id:'FN-1 · Baris 1625', type:'fn', actual:'Tinggi', pred:'Rendah',
    ...
    { id:'FN-2 · Baris 3501', type:'fn', actual:'Tinggi', pred:'Rendah',
  ...
  bva: [
    { aspect:'Fokus Pemupukan',
      before:'Takaran N, P, K diberikan merata (1:1:1) tanpa prioritas khusus.',
      after:'<strong>Fokus Nitrogen (N):</strong> Prioritaskan N karena paling sensitif terhadap perubahan status panen.' },
    { aspect:'Manajemen Air',
      before:'Irigasi dijadwalkan rutin berdasarkan waktu (pagi & sore).',
      after:'<strong>Irigasi Ambang Batas:</strong> Jaga volume di atas indeks <strong>0.591</strong> untuk komoditas tertentu.' },
    { aspect:'Kontrol Hama',
  ...
  rules: [
    { type:'ATURAN PANEN MAKSIMAL',  color:'green',
    ...
    { type:'ATURAN KOMPENSASI',      color:'yellow',
    ...
    { type:'ATURAN TOLERANSI HAMA',  color:'red',
...
const renderKPIs = kpis => {
  ...
  const cards = kpis.map(k => {
    ...
    valEl.textContent = k.counter
      ? (Number.isInteger(k.value) ? '0' : '0.00')
      : k.value;
...
const renderFeatureTable = (features, filter='all') => {
  ...
  setHTML('featureTableBody', shown.map((f, i) => `
    <tr>
      <td class="hl">${i+1}</td>
...
const renderSampleData = (data, cropMap) => {
  setHTML('sampleDataBody', data.map(r => {
...
const renderEncodings = encodings => {
  setHTML('encodingTable', encodings.map(e =>
    `<tr><td>${e.label}</td><td><span class="badge ${badgeColor(e.code)}">${e.code}</span></td></tr>`
  ).join(''));
};
...
const renderConfusionMatrices = matrices => {
  ...
  matrices.forEach(m => {
    ...
    btn.addEventListener('click', () => {
      sel.querySelectorAll('.cm-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      document.querySelectorAll('.cm-box').forEach(b =>
        b.classList.toggle('hidden', b.dataset.model !== m.model)
      );
    });
    sel.appendChild(btn);
  });
...
const renderFeatureImportance = features => {
  ...
  appendAll(grid, rows);
  setTimeout(() => grid.querySelectorAll('.fi-bar').forEach(b => { b.style.width = b.dataset.w + '%'; }), 300);
};

// ── Decision Tree Rules ──
const renderDTRules = rules => {
  setHTML('dtRules', rules.map(r => `
    <div class="dt-rule">
...
const buildYieldDonut = () => {
  const ctx = $('yieldDonutChart'); if (!ctx) return;
  new Chart(ctx, {
    type: 'doughnut',
    data: {
...
const buildMissingChart = missing => {
  const ctx = $('missingValueChart'); if (!ctx) return;
  new Chart(ctx, {
    type: 'bar',
    data: {
      labels: missing.attrs,
      datasets: [{
        label: 'Missing (%)', data: missing.pct, borderWidth:0, borderRadius:4,
...
const buildSmoteChart = () => {
  const ctx = $('smoteChart'); if (!ctx) return;
  new Chart(ctx, {
    type:'bar',
    data:{
...
document.addEventListener('DOMContentLoaded', () => {
  const { kpis, models, metrics, missing, features, encodings, sampleData, cropMap,
